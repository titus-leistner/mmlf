...
class AIO_HighPerfCouplingBlock(HighPerfCouplingBlock):
    ...
    def __init__(self, dims_in, dims_c=[], subnet_constructor=None,
        ...
        super().__init__(dims_in, dims_c=dims_c,
                         subnet_constructor=subnet_constructor, clamp=clamp)
        ...
        if act_norm_type == 'SIGMOID':
            ...
            raise ValueError(
                'Please, SIGMOID, SOFTPLUS or EXP, as actnorm type')

        assert act_norm > 0., "please, this is not allowed. don't do it. take it... and go."
        channels = self.in_channels

        self.act_norm = nn.Parameter(torch.ones(
            1, channels, 1, 1) * float(act_norm))
    ...
    def forward(self, x, c=[], rev=False):
        ...
        self.last_jac += ((-1)**rev * n_pixels) * \
            (torch.log(self.actnorm_activation(self.act_norm) + 1e-12).sum())
        return [x_out]
